...
namespace
{
	...
	{
		if (ltg.NumberOfItems() == 0 || point.positionJacobian.NumberOfColumns() == 0)
		{
			return;
		}

		CHECKandTHROW(point.positionJacobian.NumberOfColumns() == ltg.NumberOfItems(),
			"GeneralContact::ComputePotentialContactODE2RHS: inconsistent LTG and point Jacobian dimensions");

		generalizedGradient.SetNumberOfItems(ltg.NumberOfItems());
		generalizedGradient.SetAll(0.);
		EXUmath::MultMatrixTransposedVectorAdd(point.positionJacobian, pointGradient, generalizedGradient);
...
Index GeneralContact::AddRigidBodySurfaceMesh(Index rigidBodyMarkerIndex, Index frictionMaterialIndex,
	const ResizableArray<Vector3D>& pointList, const ResizableArray<Index3>& triangleList, bool staticMesh)
{
	contactIsFinalized = false;

	if (pointList.NumberOfItems() == 0)
	{
		PyError("GeneralContact: AddRigidBodySurfaceMesh(...): pointList must not be empty");
	}
	if (triangleList.NumberOfItems() == 0)
	{
	...
	for (Index i = 0; i < triangleList.NumberOfItems(); i++)
	{
		...
		for (Index j = 0; j < triangle.NumberOfItems(); j++)
		{
			if (!pointList.IsValidIndex(triangle[j]))
			{
				PyError("GeneralContact: AddRigidBodySurfaceMesh(...): triangle " + EXUstd::ToString(i) +
...
void GeneralContact::FinalizeContact(const CSystem& cSystem)//, Index3 searchTreeSize, const Matrix& frictionPairingsInit, Vector3D searchTreeBoxMin, Vector3D searchTreeBoxMax)
{
	...
	for (Index i = 0; i < potentialRigidMeshes.NumberOfItems(); i++)
	{
		...
		if (markerNumber >= cSystem.GetSystemData().GetCMarkers().NumberOfItems())
		{
			PyError("Assemble() / FinalizeContact: illegal marker number of rigidBodySurfaceMesh " + EXUstd::ToString(i));
		}
		else if (!EXUstd::IsOfType((Index)cSystem.GetSystemData().GetCMarkers()[markerNumber]->GetType(),
			(Index)Marker::Position + (Index)Marker::Orientation))
		{
			PyError("Assemble() / FinalizeContact: rigidBodySurfaceMesh " + EXUstd::ToString(i) +
				" has invalid marker: must be of Position+Orientation");
		}

		cSystem.GetSystemData().ComputeMarkerODE2LTGarray(markerNumber, mesh.state.markerLTG, true);
	}
...
void GeneralContact::UpdatePotentialRigidMeshes(const CSystem& cSystem, bool computeJacobians)
{
	...
	for (Index meshIndex = 0; meshIndex < potentialRigidMeshes.NumberOfItems(); meshIndex++)
	{
		...
		mesh.state.markerOrientation = markerData.orientation;
		mesh.state.markerVelocity = markerData.velocity;
		mesh.state.markerAngularVelocityLocal = markerData.angularVelocityLocal;
		if (computeJacobians)
		{
			mesh.state.markerPositionJacobian.CopyFrom(markerData.positionJacobian);
			mesh.state.markerRotationJacobian.CopyFrom(markerData.rotationJacobian);
			systemData.ComputeMarkerODE2LTGarray(mesh.markerIndex, mesh.state.markerLTG, true);
		}
		else
		{
			mesh.state.markerPositionJacobian.SetNumberOfRowsAndColumns(0, 0);
			mesh.state.markerRotationJacobian.SetNumberOfRowsAndColumns(0, 0);
		}

		mesh.vertexKinematics.SetNumberOfItems(mesh.verticesLocal.NumberOfItems());
		for (Index vertexIndex = 0; vertexIndex < mesh.verticesLocal.NumberOfItems(); vertexIndex++)
		{
			const Vector3D& xLocal = mesh.verticesLocal[vertexIndex].xLocal;
			auto& vertex = mesh.vertexKinematics[vertexIndex];
			vertex.position = PotentialContact::ComputeVertexPosition(mesh.state, xLocal);
			vertex.velocity = PotentialContact::ComputeVertexVelocity(mesh.state, xLocal);
			if (computeJacobians)
			{
				PotentialContact::ComputeVertexPositionJacobian(mesh.state, xLocal, vertex.positionJacobian);
			}
			else
			{
...
void GeneralContact::ComputePotentialContactODE2RHS(const CSystem& cSystem, TemporaryComputationDataArray& tempArray, Vector& systemODE2Rhs)
{
	...
	{
		item->sparseVector.SetVectorSize(systemODE2Rhs.NumberOfItems());
		item->sparseVector.SetAllZero();
	}
	...
	for (const EXUmath::IndexValue& item : sparseVector.GetSparseIndexValues())
	{
		systemODE2Rhs[item.GetIndex()] -= item.GetValue();
	}
...
void GeneralContact::ComputeODE2RHS(const CSystem& cSystem, TemporaryComputationDataArray& tempArray, Vector& systemODE2Rhs)
{
	...
	if (settings.contactFormulation != ContactFormulation::Penalty)
	{
		ComputePotentialContactODE2RHS(cSystem, tempArray, systemODE2Rhs);
		return;
	}
